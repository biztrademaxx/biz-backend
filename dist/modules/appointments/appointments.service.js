...
async function listEventAppointments(params) {
    ...
    const booths = boothPairs.length > 0
        ? await prisma_1.default.exhibitorBooth.findMany({
            where: {
                OR: boothPairs.map((p) => ({
                    eventId: p.eventId,
        ...
        : [];
    const boothByKey = new Map(booths.map((b) => [`${b.eventId}:${b.exhibitorId}`, b.boothNumber]));
