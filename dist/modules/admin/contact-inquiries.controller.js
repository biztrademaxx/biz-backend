...
exports.listContactInquiriesHandler = listContactInquiriesHandler;
const prisma_1 = __importDefault(require("../../config/prisma"));
/**
 * GET /api/admin/contact-inquiries?limit=&offset=
 * Lists public contact form rows (newest first).
 */
async function listContactInquiriesHandler(req, res) {
    try {
        ...
        const [rows, total] = await Promise.all([
            prisma_1.default.contactInquiry.findMany({
                orderBy: { createdAt: "desc" },
                take: limit,
                skip: offset,
            }),
            prisma_1.default.contactInquiry.count(),
        ]);
        res.json({
            success: true,
            data: rows.map((r) => ({
