...
const organizers_list_query_1 = require("../../middleware/organizers-list-query");
...
async function getOrganizersHandler(req, res) {
    try {
        const result = await (0, organizers_service_1.listOrganizers)((0, organizers_list_query_1.organizersListOptionsFromRequest)(req));
        return res.json({
...
async function createOrganizerPromotionHandler(req, res) {
    try {
        ...
        const body = req.body;
        const result = await (0, organizers_service_1.createOrganizerPromotion)(id, auth.sub, body);
        if ("error" in result) {
