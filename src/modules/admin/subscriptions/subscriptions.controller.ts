// C:\bizz\biz-backend\src\modules\subscriptions\subscriptions.controller.ts
...
export async function getOrganizerPlansBatchHandler(req: Request, res: Response) {
  try {
    const userId = req.auth?.sub;
    if (!userId) {
      return res.status(401).json({ message: "Unauthorized" });
    ...
    if (!organizerIds || !Array.isArray(organizerIds) || organizerIds.length === 0) {
    ...
    const result = await getOrganizerPlansBatch({
      userId,
      organizerIds,
      isAdmin,
    });
    ...
    console.error("getOrganizerPlansBatchHandler:", error);
    return res.status(500).json({ message: "Failed to fetch organizer plans" });
  }
}