...
type VenueRow = Prisma.UserGetPayload<{ select: typeof venueSelect }>;

function mapVenueToDto(v: VenueRow) {
  const now = new Date();
...
function buildVenueWhere(search: string, status?: string): Prisma.UserWhereInput {
  const now = new Date();
...
export async function listVenueEventsForAdmin(query: Record<string, unknown> = {}) {
  const { page, limit, search, status, skip } = parseListQuery(query);
  const now = new Date();
  ...
  const [venues, total, totalVenues, totalEvents, activeEvents] = await Promise.all([
    ...
    prisma.event.count({ where: { venueId: { not: null } } }),
    prisma.event.count({
